...
window.addEventListener('trige',function(e){
	var wvs = plus.webview.all();
	var indexview = plus.webview.getLaunchWebview();
	var myHtml = plus.webview.getWebviewById('../customerInfo/myAcont.html');
	var personalInformationHomeHtml = plus.webview.getWebviewById("../customerInfo/personalInformationHome.html");
	if(myHtml){
		for(var i = 0; i < wvs.length; i++) {
			//console.log("webview"+i+": "+wvs[i].getURL());
			if(wvs[i] != indexview&&wvs[i] != myHtml && wvs[i] != personalInformationHomeHtml ) {
				wvs[i].close();
			}
		}
	}else{
	...
	mui.trigger(document.getElementById('account'),"tap");
 ...
 */
window.addEventListener('myFour',function(){
	var personalInformationHomeHtml = plus.webview.getWebviewById("../customerInfo/personalInformationHome.html");
        personalInformationHomeHtml.close();                        
         setTimeout(function(){
               mui.trigger(document.getElementById('personalInformationHome'),"tap");                                
         },100);
});
