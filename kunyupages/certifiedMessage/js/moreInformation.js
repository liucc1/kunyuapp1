...
$("#oBtn").on("tap",function(){
	...
	if(!telExp.test(unitPhone)) {
		mui.toast("单位电话号格式不正确,区号应为3至4位，首位为0");
		return false;
	};
    ...
    for (var i=0;i<house.length;i++) {
    	...
    	if(houseValues[i].value <= 0){
			mui.toast("房产价值不能小于等于零");
			return false;
		}
		var reg = /^[1-9]\d{0,8}$/
		if(!reg.test(houseValues[i].value.split(".")[0])){
			mui.toast("房产价值格式有误，最多输入九位数");
			return false;
		}
...
//	    var bianhao = /^[0-9|a-z|A-Z]{5,30}$/
//		if(!bianhao.test(houseId[i].value.split(".")[0])){
//			mui.toast("房产编号有误，至少输入五位数");
//			return false;
//		}
