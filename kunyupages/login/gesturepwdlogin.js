		...
		eg.generalPostAjax(url,params,function(data){
			...
			if(data.resCode == "USPB0005"){
						...
						if(!eg.isEmpty(errorCount)){
							...
							if(errorCount == "3"){
								mui.toast("您已输错3次手势锁密码，请用其他方式登录！",{duration: 'short',type: 'div'});
...
//										mui.fire(details,"goLogin",{});
						...
						mui.toast("您已输错"+errorCount+"次手势锁密码!",{duration: 'short',type: 'div'});
						return;
					}
			 if( data.resCode !== "0" ){
					mui.toast(data.resMsg,{duration: 'short',type: 'div'});
					return;
				}
