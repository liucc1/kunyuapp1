...
$("#oBtn").on('tap',function(){
	...
	if(!eg.name.test(name)){
		mui.toast("姓名格式不正确！",{duration: 'short',type: 'div'});
		return false;
	}
	if(isNullVal(idNo)){
		mui.toast("身份证不能为空！",{duration: 'short',type: 'div'});
		return false;
	}
	if(!eg.userIdCode(idNo)){
		mui.toast("身份证格式不正确！",{duration: 'short',type: 'div'});
		return false;
	}
//	if(!ifFlag){
//		mui.toast("请拍摄身份证反面！",{duration: 'short',type: 'div'});
//		return false;
//	}
	if(isNullVal(cardNo)){
