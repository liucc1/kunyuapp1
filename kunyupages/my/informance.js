...
$("#oBtn").on('tap',function(){
	...
	if(isNullVal(name)){
		mui.toast("姓名不能为空！",{duration: 'short'});
		return false;
	}
	if(!eg.name.test(name)){
		mui.toast("姓名格式不正确！",{duration: 'short'});
		return false;
	}
	if(isNullVal(idNo)){
		mui.toast("身份证不能为空！",{duration: 'short'});
		return false;
	}
	if(!eg.userIdCode(idNo)){
		mui.toast("身份证格式不正确！",{duration: 'short'});
		return false;
	}
//	if(!ifFlag){
//		mui.toast("请拍摄身份证反面！",{duration: 'short'});
//		return false;
//	}
	if(isNullVal(cardNo)){
	...
	eg.postAjax2("user/data", params, function(data){
		if(data.status == 1){
			mui.toast("已绑卡成功",{duration: 'short'});
