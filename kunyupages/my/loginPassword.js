...
$('#oBtn').click(function() {
	if(!beforPwdVal) {
		mui.toast("旧登录密码不能为空！",{duration: 'short',type: 'div'});
		return false;
	};
	if(!afterPwdVal) {
		mui.toast("新登录密码不能为空！",{duration: 'short',type: 'div'});
		return false;
	};
	var ok = plus.pluginPGKeyboard.checkMatch("afterPwd");
	if(!ok){
    	mui.toast("您输入的新密码格式不正确",{duration: 'short',type: 'div'});
    	return false;
   	}
	if(beforPwdVal == afterPwdVal){
		mui.toast("新密码与旧密码不能相同！",{duration: 'short',type: 'div'});
		return false;
	}
	...
	eg.postAjax2("chgpass",params, function(data) {
		if(data.status == "1" ){
			mui.toast("密码修改成功，请重新登录",{duration: 'short',type: 'div'});
